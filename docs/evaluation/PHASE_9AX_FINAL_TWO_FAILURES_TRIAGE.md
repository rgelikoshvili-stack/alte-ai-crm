...
| `english_program_requirements_en` | `What are the English-language program requirements if they are in the approved source?` | International Admissions / `international_admissions_sources` | Programs / `programs` | English-language requirement wording was treated as generic Programs unless international/applicant terms were present; after the first 9AX deploy, source selection was fixed but final department routing still chose Programs from the word `program`. | Added English-language program requirements priority detection for source selection and final department resolver routing. | Could over-route generic program browsing to International Admissions. Preserved generic programs test. | `test_english_program_requirements_routes_to_international_not_programs`, `test_english_program_requirements_department_resolver_routes_to_international`, IELTS/TOEFL variants, generic programs preservation test. |
