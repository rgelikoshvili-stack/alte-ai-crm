...
function LeadModal({ S, lang, onClose, onSubmit }){
  React.useEffect(ensureModalCss,[]);
  const [name, setName] = useState2(lang==='KA'?'ნინო კვარაცხელია':'Nino Kvaratskhelia');
  const [phone, setPhone] = useState2('+995 599 12 34 56');
    ...
    <div className="cw-overlay" onClick={onClose}>
      <div className="cw-modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="body">
          ...
          <button className="submit" disabled={!valid} onClick={onSubmit}>
