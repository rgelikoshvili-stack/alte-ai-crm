...
Phase 9K fixes were redeployed to Cloud Run as image `v0.9-security-reliability-fixes`, revision `alte-ai-crm-backend-00007-xmp`. Security/reliability smoke passed `16/16`; department routing smoke passed `28/28`; finance no-contact smoke passed `24/24`; broader knowledge smoke passed `25/25`. Public launch remains `NOT_COMPLETE`, actual site embed remains blocked, and `#privacy-policy-pending` remains a launch blocker until the official privacy URL is approved.
Phase 9L-M-N final approval intake and handoff package are prepared. No approval has been inferred or marked complete. Asset upload, actual site embed, real-domain smoke, and public launch remain blocked until explicit approvals are recorded.
Phase 9L-P records the current approval/access state: conservative content policy is ready but final human review for sensitive exact facts remains pending; privacy/data is approved in principle but the official privacy URL remains pending; website access is approved for preparation but actual upload/embed remains pending. Public launch remains NO-GO pending site embed and real-domain smoke.
Phase 9N-Test prepared a separate standalone test site package using the production backend and Safe Pro Sidebar widget. API smoke passed `10/10`; browser smoke remains pending until a local/manual or approved hosted test origin is used. Real Alte site, production DB, Cloud Run, Secret Manager, and public launch state remain unchanged.
Phase 9N-CORS prepared the temporary hosted test-origin approval gate. The test origin URL is pending, production CORS was not changed, Cloud Run was not redeployed, hosted browser smoke was not executed, and the real Alte site remains untouched.
Phase 9N-CORS-Execution added the exact temporary origin `https://alte-ai-chat-test.netlify.app` to production CORS. Cloud Run serving revision is `alte-ai-crm-backend-00009-bhk`; backend image remains `v0.9-security-reliability-fixes`. CORS smoke passed, and safe API smokes passed. Hosted browser smoke remains pending manual verification. Public launch remains NO-GO.
Phase 9N-CORS-Actual-Netlify-Origin added the actual deployed Netlify origin `https://nimble-croissant-2f66e8.netlify.app` to production CORS while preserving the previous test origin and Alte origins. Cloud Run serving revision is `alte-ai-crm-backend-00010-g47`; backend image remains `v0.9-security-reliability-fixes`. CORS smoke passed `10/10`, and safe no-contact API smokes passed. Hosted browser smoke remains pending manual retest. Public launch remains NO-GO.
Phase 9T official Alte 8 PDF Knowledge Base package is prepared from the 8 official PDFs. Normalized JSONL has `273` chunks, including `56` review-required chunks; question bank, taxonomy, answer policy, reviewer CSV, behavior docs, and dry-run apply script are ready. Dry-run apply completed with `would_write=false`; production DB was not modified and `--apply` was not run. Public launch remains NO-GO pending human review and explicit DB apply approval.
Phase 9Z chatbot-required knowledge package is prepared from the larger local Alte documents export and applied to production Knowledge Base tables after explicit user approval. Only student/applicant-facing required sources were selected; internal-only administrative documents were skipped unless directly useful for chatbot users. The package contains `433` Georgian Q&A records with source, topic, department, and answer policy metadata plus a `433` row reviewer queue and safe apply script. Coverage was expanded with payment/finance routing, state/social grants, formal communication, international student routing, IT support, and student service department content. Production apply created `433` KnowledgeSource and `433` KnowledgeSnippet records, then approved all `433` for chatbot retrieval while retaining review-required metadata on `360` sensitive/cautious records. Post-apply safe smokes passed: session payload `2/2`, test site API `10/10`, and production knowledge `25/25`. Public launch remains NO-GO pending website/privacy/embed approvals and final launch approval.
...
Cloud Run service `alte-ai-crm-backend` was redeployed with image `v0.9-department-routing-sidebar`.
...
- No contact details sent
- Contact-flow test not run
- No intentional production customers/leads/tasks created

Readiness decision:

- Backend remains deployed.
- Finance no-contact guard remains verified.
- Department routing for ambiguous sidebar-only Finance/Medicine messages is not yet verified.
- Actual site embed and public launch remain blocked.

Required follow-up:

- Fix sidebar context priority for ambiguous messages.
- Redeploy and rerun department routing smoke.
