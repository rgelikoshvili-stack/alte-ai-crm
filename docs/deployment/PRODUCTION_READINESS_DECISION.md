...
Phase 9K has local code/docs/tests/verifier fixes for AI provider fallback, public handover spam guard, RBAC deny-by-default, and production auth validation. These fixes require redeploy before production behavior changes. Public launch remains `NOT_COMPLETE`, actual site embed remains blocked, and `#privacy-policy-pending` remains a launch blocker until the official privacy URL is approved.
