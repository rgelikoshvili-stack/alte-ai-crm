...
Current decision: `BACKEND_DEPLOYED_CHATBOT_REQUIRED_KNOWLEDGE_READY_PENDING_REVIEW_AND_APPLY`
...
Phase 9Z chatbot-required knowledge package is prepared from the larger local Alte documents export. Only student/applicant-facing required sources were selected; internal-only administrative documents were skipped unless directly useful for chatbot users. The package contains `433` Georgian Q&A records with source, topic, department, and answer policy metadata plus a `433` row reviewer queue and dry-run apply script. Coverage was expanded with payment/finance routing, state/social grants, formal communication, international student routing, IT support, and student service department content. Production DB was not modified and no DB apply/import was run. Public launch remains NO-GO pending review and explicit DB apply approval.
