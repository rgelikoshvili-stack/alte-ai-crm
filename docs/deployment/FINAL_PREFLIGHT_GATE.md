...
Phase 9V Netlify chatbot CORS restore is prepared. The active CORS allowlist already includes `https://nimble-croissant-2f66e8.netlify.app`; `/chat/message` preflight returns the exact origin and no wildcard. CORS middleware was moved outermost so backend error responses can include CORS headers. Production DB and Secret Manager were not changed.
