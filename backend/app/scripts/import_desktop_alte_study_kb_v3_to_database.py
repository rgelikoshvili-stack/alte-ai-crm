...
SEED_PATH = BACKEND_ROOT / "app" / "knowledge_seed" / "alte_desktop_study_kb_v3" / "alte_kb_complete_v3_normalized.jsonl"
...
async def import_records(*, seed_path: Path = SEED_PATH, apply: bool = False, allow_production: bool = False) -> dict[str, Any]:
...
async def upsert_source(db, entry: dict[str, Any]) -> tuple[KnowledgeSource, bool, bool]:
    ...
    values = {
        ...
        "owner": "phase_desktop_study_kb_v3_import",
    }
    if source is None:
...
def source_title(entry: dict[str, Any]) -> str:
    return f"{entry.get('title') or entry['source_key']} ({entry['category']})"[:255]
...
async def _amain() -> int:
    parser = argparse.ArgumentParser(description="Import desktop Alte study KB v3 into the app Knowledge Base.")
    ...
    summary = await import_records(apply=args.apply, allow_production=args.allow_production)
