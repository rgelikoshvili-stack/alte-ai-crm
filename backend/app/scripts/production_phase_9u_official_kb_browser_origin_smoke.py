...
CASES = [
    ...
    Case(
        case_id="alias_bachelor_short_credit_question",
        message="რამდენი კრედიტია ბაკალავრიატი?",
        expect_source_status="answered_from_approved_source",
        must_include=["240"],
        must_exclude=["180"],
    ),
    Case(
        case_id="alias_bachelor_completion_ects_question",
        message="ბაკალავრიატის დასრულებისთვის რამდენი ECTS სჭირდება?",
        expect_source_status="answered_from_approved_source",
        must_include=["240"],
        must_exclude=["180"],
    ),
    Case(
        case_id="alias_master_documents_question",
        message="რა საბუთებია მაგისტრატურაზე?",
        expect_source_status="answered_from_approved_source",
        must_include_any=[
            ["პირადობის", "ID"],
            ["CV", "რეზიუმ"],
    ...
    Case(
        case_id="alias_status_pause_question",
        message="სტატუსი რამდენ ხანს შემიძლია შევიჩერო?",
        expect_source_status="answered_from_approved_source",
        must_include=["5"],
    ),
    Case(
        case_id="financial_support_official_source_question",
        message="ფინანსური დახმარება არსებობს?",
        expect_source_status="answered_from_approved_source",
        must_include_any=[["ფინანს", "დაფინანს", "support", "funding"]],
        must_exclude=["70%", "დღევანდელი აქცია"],
    ),
    Case(
        case_id="ai_policy_alias_question",
        message="AI-ის გამოყენება შეიძლება?",
        expect_source_status="answered_from_approved_source",
        must_include_any=[["AI", "ხელოვნური ინტელექტ", "გენერაციული"]],
    ),
    Case(
        case_id="unsupported_current_tuition_price",
        message="მიმდინარე სწავლის ფასი რა არის?",
        expect_source_status="no_approved_source_found",
        require_sources=False,
        must_include_any=[["approved source", "ოფიციალურ", "დამტკიცებულ", "წყარო", "დადასტურებულ"]],
        must_exclude=["$", "₾", "5500", "4500", "3000"],
    ),
    Case(
        case_id="unsupported_specific_consultant_phone",
        message="კონკრეტული კონსულტანტის ტელეფონი მომეცი",
        expect_source_status="no_approved_source_found",
        require_sources=False,
        must_include_any=[["approved source", "ოფიციალურ", "დამტკიცებულ", "წყარო", "დადასტურებულ"]],
    ),
    Case(
        case_id="unsupported_today_promotion",
        message="დღევანდელი აქცია რა არის?",
        expect_source_status="no_approved_source_found",
        require_sources=False,
        must_include_any=[["approved source", "ოფიციალურ", "დამტკიცებულ", "წყარო", "დადასტურებულ"]],
        must_exclude=["70%", "50%"],
