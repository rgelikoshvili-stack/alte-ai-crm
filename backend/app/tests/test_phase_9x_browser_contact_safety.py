...
def test_georgian_plural_contact_request_fragment_is_removed(client, session_factory, monkeypatch):
    patch_analysis(
        monkeypatch,
        AIAnalysisResult(
            reply="გაგიმარჯოთ! გთხოვთ, მოგვაწოდოთ თქვენი საკონტაქტო ინფორმაცია, რომ ოპერატორი დაგიკავშირდეთ.",
            language="ka",
            intent="human_request",
            confidence=0.95,
            should_create_lead=False,
    ...
    assert "გთხოვთ, მოგვაწოდოთ თქვენი" not in result["reply"]
    assert "საკონტაქტო ინფორმაციის გაზიარება მხოლოდ თქვენი მკაფიო თანხმობის შემდეგ უნდა მოხდეს." in result["reply"]
    assert_no_crm_side_effects(result, session_factory)
