  ...
  return (
    <div className={"cw-row "+(isUser?'u':'')}>
      <div className={"cw-av "+(isUser?'u':isOperator?'op':'')}>
        {isUser ? (S.you||'ნ')[0] : isOperator ? <I name="headset" size={14} sw={2.3}/> : <AlteMark size={26}/>}
      </div>
...
function ChatWidget({ S, lang, setLang, tweaks, onClose, expanded, setExpanded }){
    ...
    if (detectIntent(text) === 'handover'){
      ...
      try {
        ...
        setMessages(m => [...m, {
          id:'a'+Date.now(),
          role:'assistant',
          text: lang==='KA' ? 'ოპერატორთან გადართვა ვერ მოხერხდა. სცადე მოგვიანებით.' : 'Could not request an operator right now. Please try again later.',
          deptId:currentDept,
        }]);
      }
      return;
  ...
  const onLeadSubmit = async (contact) => {
    try {
      ...
      setMessages(m => [...m, {
        id:'a'+Date.now(),
        role:'assistant',
        text: lang==='KA' ? 'კონტაქტის გაგზავნა ვერ მოხერხდა. გადაამოწმე ველები და სცადე თავიდან.' : 'Could not send your contact request. Check the fields and try again.',
        deptId:currentDept,
      }]);
    }
  };
  ...
  const syncOperatorMessages = useCallback(async () => {
    if (!window.AlteChatBackend?.pollOperatorMessages) return undefined;
    const rows = await window.AlteChatBackend.pollOperatorMessages().catch(()=>[]);
    const fresh = rows.filter(row => !seenOperatorMessageIds.current.has(row.id));
    if (!fresh.length) return;
    fresh.forEach(row => seenOperatorMessageIds.current.add(row.id));
    setMessages(m => [
      ...m,
      ...fresh.map(row => ({
        id:'op'+row.id,
  ...
  useEffect(() => {
    const timer = setInterval(() => { syncOperatorMessages(); }, 2500);
    window.addEventListener('focus', syncOperatorMessages);
    document.addEventListener('visibilitychange', syncOperatorMessages);
    return () => {
      clearInterval(timer);
      window.removeEventListener('focus', syncOperatorMessages);
      document.removeEventListener('visibilitychange', syncOperatorMessages);
    };
  }, [syncOperatorMessages]);
